#!/bin/bash
# run_server.sh - Start RMI Registry and Password Manager Server (Rebuild)
...
# --- Start Password Manager Server (which now starts the registry) ---
echo "Starting Password Manager Server (with embedded RMI registry)..."
...
"${JAVA}" \
    ...
    com.example.passwordmanager.Server

# Check if the server started successfully (the Java command itself might error out)
if [ $? -ne 0 ]; then
    echo "Error: Failed to start Password Manager Server. Check Java output above for details."
    exit 1
fi
